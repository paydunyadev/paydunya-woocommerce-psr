...
function woocommerce_paydunya_init()
{
    ...
    class WC_Paydunya extends WC_Payment_Gateway
    {
        public function __construct()
        {
            ...
            wp_register_script('custom-js', WP_PLUGIN_URL.'/PLUGIN_NAME/js/custom.js', [], null, true);
            wp_enqueue_script('custom-js');
            $wnm_custom = ['template_url' => get_bloginfo('url')];
            ...
            add_action('woocommerce_api_'. strtolower(get_class($this)), [$this, 'callback_handler']);
        }

        public static function paydunya_callback()
        {
            $WC_Paydunya = new WC_Paydunya();

            try {
                if ($_POST['data']['hash'] === hash('sha512', $WC_Paydunya->settings['master_key'])) {
                    if ($_POST['data']['status'] == "completed") {
                        ...
                        $order->add_order_note($WC_Paydunya->msg['message']);
        ...
        public static function paydunya_api()
        {
            $ch = curl_init();
            ...
            $json = json_encode($WC_Paydunya->get_paydunya_args($order));

            $master_key = $WC_Paydunya->settings['master_key'];
            if ($WC_Paydunya->settings['sandbox'] == "yes") {
                $url = 'https://app.paydunya.com/sandbox-api/v1/checkout-invoice/create';
                $private_key = $WC_Paydunya->settings['test_private_key'];
                $token = $WC_Paydunya->settings['test_token'];
            } else {
                $url = 'https://app.paydunya.com/api/v1/checkout-invoice/create';
                $private_key = $WC_Paydunya->settings['live_private_key'];
                $token = $WC_Paydunya->settings['live_token'];
            }
            ...
            $mode = $WC_Paydunya->settings['sandbox'] == 'yes'
                ? 'test'
                : 'live';
        ...
        public function admin_options()
        {
            ...
            echo '</table>';
            wp_enqueue_script('paydunya_admin_option_js', plugin_dir_url(__FILE__) . 'assets/js/settings.js', ['jquery'], '1.0.1');
        }
        ...
        protected function get_paydunya_args($order)
        {
            ...
            $redirect_url = function_exists('wc_get_cart_url')
                ? wc_get_checkout_url()
                : $woocommerce->cart->get_checkout_url();

            $productinfo = "Commande: " . $order->get_id();

            $str = "$txnid|$order->get_total()|$productinfo|$order->get_billing_first_name()|$order->get_billing_email()";
        ...
        public function post_to_url($url, $data, $order_id)
        {
        ...
        public function check_paydunya_response($mtoken)
        {
            global $woocommerce;
            if ($mtoken <> "") {
                ...
                try {
                    ...
                    if ($respond_code == "00") {
                        ...
                        if ($status == "completed") {
                            ...
                            $customer = trim($order->get_billing_last_name() . " " . $order->billing_first_name());
                            if ($this->sms == "yes") {
        ...
        public static function add_paydunya_fcfa_currency_symbol($currency_symbol, $currency)
        {
            switch (
            $currency) {
        ...
        public static function woocommerce_add_paydunya_gateway($methods)
        {
            $methods[] = 'WC_Paydunya';
            return $methods;
        }
    ...
    add_filter('woocommerce_currencies', ['WC_Paydunya', 'add_paydunya_fcfa_currency']);
    add_filter('woocommerce_currency_symbol', ['WC_Paydunya', 'add_paydunya_fcfa_currency_symbol'], 10, 2);
    add_filter("plugin_action_links_$plugin", ['WC_Paydunya', 'woocommerce_add_paydunya_settings_link']);
    add_filter('woocommerce_payment_gateways', ['WC_Paydunya', 'woocommerce_add_paydunya_gateway']);

    add_action('rest_api_init', function () {
        register_rest_route('wp/v1', '/paydunya-api/', [
            'methods' => 'GET',
            'callback' => ['WC_Paydunya','paydunya_api'],
        ]);
        register_rest_route('wp/v1', '/paydunya-callback/', [
            'methods' => 'POST',
            'callback' => ['WC_Paydunya','paydunya_callback'],
        ]);
    });
}
